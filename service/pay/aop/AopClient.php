...
//../f2fpay/config/rsa_private_key.pem
	//私钥值
	...
	function verify($data, $sign, $rsaPublicKeyFilePath, $signType = 'RSA') {
		...
		if($this->checkEmpty($this->alipayPublicKey)){
			...
			$res = "-----BEGIN PUBLIC KEY-----\n" .
				wordwrap($pubKey, 64, "\n", true) .
				"\n-----END PUBLIC KEY-----";
			Yii::$app->log->logger->log($data,Logger::LEVEL_INFO);
			Yii::$app->log->logger->log($sign, Logger::LEVEL_INFO);
            Yii::$app->log->logger->log($pubKey, Logger::LEVEL_INFO);
            Yii::$app->log->logger->log($res, Logger::LEVEL_INFO);
		}else {
			//读取公钥文件
