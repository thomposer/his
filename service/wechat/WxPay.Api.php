...
//		$inputObj->SetAppid(WxPayConfig::$APPID);//公众账号ID
        ...
        $inputObj->SetAppid($config ? $config['appid'] : ''); //公众账号ID
        $inputObj->SetMch_id($config ? $config['mchid'] : ''); //商户号
...
//		curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,2);//严格校验
        ...
        if ($useCert == true) {
            $cacheKey = Yii::getAlias('@wxPayConfig') . $_COOKIE['wechatSpotId'];
            $config = Yii::$app->cache->get($cacheKey);
            $certFilePath = $config ? Yii::getAlias('@ServicePath') . '/wechat/cert/' . $config['appid'] . '/' : '';
            $certFile = '';
            $keyFile = '';
            if ($certFilePath) {
                $certFile = $certFilePath . 'apiclient_cert.pem';
                $keyFile = $certFilePath . 'apiclient_key.pem';
            }
            //设置证书
