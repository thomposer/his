...
class MicroPay
{
	...
	public function pay($microPayInput)
	{
		...
		$result = WxPayApi::micropay($microPayInput, 5);
                \Yii::info('WxPayApi micropay result ['.var_export($result,true).']');
		//如果返回成功
		if(!array_key_exists("return_code", $result)
//			|| !array_key_exists("out_trade_no", $result)) //需要输入支付密码的时候   第一次返回没有这个值 暂时先去掉
		...
		while($queryTimes > 0)
		{
			...
			$queryResult = $this->query($out_trade_no, $succResult);
                        \Yii::info('WxPayApi queryResult ['.var_export($queryResult,true).']');
			//如果需要等待1s后继续
			if($succResult == 2){
                            $queryTimes--;
                            if($qStatus==1){
