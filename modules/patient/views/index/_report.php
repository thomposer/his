            ...
            if (!empty($inspectCheckList)) {
                foreach ($inspectCheckList as $key => $val) {
                    ...
                    if (isset($val['inspectUnionDataProvider'])) {//实验室检查 有列表
                        echo Html::tag('div', $this->render('@inspectReport', [
                                    'val' => $val,
                                    'hidden' => true,
                                    'status' => 1,
                        ...
                        echo Html::tag('div', $this->render('@checkReport', [
                                    'val' => $val,
                                    'hidden' => true,
                                    'status' => 1,
...
<?php
    $this->registerJs("
        $(\"#ajaxCrudModal .modal-content .patient-form .kv-file-remove\").hide();
        $(\"#ajaxCrudModal .modal-content .patient-form .kv-file-download i\").removeClass('fa-eye');
        $(\"#ajaxCrudModal .modal-content .patient-form .kv-file-download i\").addClass('fa-download');
    ");
?>