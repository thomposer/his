 ...
 */
class MaterialOutboundSearch extends MaterialOutbound
{
    ...
    public function searchDrugs($params, $pageSize = 20) {
        ...
        $query->select(['a.id', 'b.name', 'b.specification', 'b.manufactor', 'a.material_outbound_id', 'a.num', 'c.outbound_time', 'userName' => 'e.username', 'leadingUser' => 'g.username', 'c.status', 'department_name' => 'd.name']);
        $query->leftJoin(['f' => MaterialStockInfo::tableName()], '{{a}}.material_stock_info_id={{f}}.id');
        $query->leftJoin(['b' => Material::tableName()], '{{f}}.material_id = {{b}}.id');
        $query->leftJoin(['c' => MaterialOutbound::tableName()], '{{a}}.material_outbound_id= {{c}}.id');
        ...
        $dataProvider = new ActiveDataProvider([
            ...
            'sort' => [
                'defaultOrder' => [
                    'material_outbound_id' => SORT_DESC,
                    'id' => SORT_ASC,
                ],
                'attributes' => ['id', 'material_outbound_id', 'outbound_time']
            ]
        ]);
