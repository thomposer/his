...
<div class="stock-form col-md-12">
    ...
    <div class = 'box'>
        <?= $form->field($model->getModel('stockInfo'), 'materialName')->dropDownList(ArrayHelper::map($materialList, 'id', 'name'), ['prompt' => '请选择', 'class' => 'form-control select2', 'style' => 'width:100%'])->label(false) ?>
        <?=
        GridView::widget([
            ...
            'columns' => [
                ...
                [
                    ...
                    'value' => function ($dataProvider) {
                        return Html::input('text', 'MaterialStockInfo[total_num][]', $dataProvider->total_num, ['class' => 'form-control stock-inbound-focusout total_num-focusout']);
                    }
                ],
                [
                    'attribute' => 'unit',
                ...
                [
                    ...
                    'headerOptions'=>['class'=>'col-xs-1 col-sm-1 col-md-1'],
                    'value'=>function($dataProvider){
                        return Html::input('text','MaterialStockInfo[invoice_number][]',$dataProvider->invoice_number,['class'=>'form-control']);
                    }
                ],
                [
                    ...
                    'headerOptions' => ['class' => 'col-xs-1 col-sm-1 col-md-1'],
                    'value' => function($dataProvider) {
                        return Html::input('text', 'MaterialStockInfo[default_price][]', $dataProvider->default_price, ['class' => 'form-control stock-inbound-focusout default_price-focusout']);
                    }
                ],
                [
                    ...
                    'headerOptions' => ['class' => 'col-xs-2 col-sm-2 col-md-2'],
                    'value' => function($dataProvider) {
                        return '<div class="date">' . Html::input('text', 'MaterialStockInfo[expire_time][]', date('Y-m-d', $dataProvider->expire_time), ['class' => 'form-control']) . '</div>';
                    }
                ],
                [
                    ...
                    'buttons' => [
                        'delete' => function($url, $model, $key) {
                            $html = '';
                            $html .= Html::hiddenInput('MaterialStockInfo[materialStockInfoId][]', $model->id);
                            $html .= Html::hiddenInput('MaterialStockInfo[material_id][]', $model->material_id);
                            $html .= Html::hiddenInput('MaterialStockInfo[deleted][]','',['class'=>'stock-inbound-delete']) . Html::img(Yii::$app->request->baseUrl . '/public/img/common/delete.png');
                            return $html;
                        }
                    ]
                ],
    ...
    <div>
        <?= Html::a('取消',['material-inbound-index'], ['class' => 'btn btn-cancel btn-form second-cancel']) ?>
