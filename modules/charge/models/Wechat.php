...
class Wechat extends \yii\db\ActiveRecord
{
    ...
    protected function setSpotInfo() {
        ...
        if ($requestData['return_code'] == 'SUCCESS') {//回调成功
            ...
            $list = PaymentConfig::getConfig($requestData['out_trade_no'], 1);
            Yii::info('paymentConfigList:[' . json_encode($list . ']'));
            if ($list) {
                $expireTime = time() + Yii::getAlias('@loginCookieExpireTime');
                setcookie('wechatSpotId', $list['spot_id'], $expireTime, '/', null, null, true);
            }
        }
    }
    ...
    public function modifyOrder($data) {
        ...
        PaymentLog::addOrderLogByWechat($data, 3);
    }
