    ...
    <div class = 'row search-margin'>
        <div class = 'col-sm-5 col-md-5'>
            <?php  if(isset($this->params['permList']['role'])||in_array($this->params['requestModuleController'].'/create', $this->params['permList'])):?>
                <?= Html::a("<i class='fa fa-plus'></i>新增预约", ['create','return' => 'appointment-detail'], ['class' => 'btn btn-default font-body2','data-pjax' => 0]) ?>
            <?php endif?>
        ...
        <div class = 'col-sm-7 col-md-7'>
            <?php echo $this->render('_appointmentDetailSearch', ['model' => $searchModel,'doctorInfo' => $doctorInfo,'spot_type' => $spot_type]); ?>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    var apiAppointmentGetAppointmentDetail = '<?= Url::to(['@apiAppointmentGetAppointmentDetail']); ?>';
    var apiAppointmentMessage = '<?= Url::to(['@apiAppointmentMessage']); ?>';
    var apiAppointmentDoctorTimeList = '<?= Url::to(['@apiAppointmentDoctorTimeList']); ?>';
    ...
    require(["<?= $baseUrl ?>" + "/public/js/make_appointment/appointmentDetail.js?v="+'<?= $versionNumber ?>'], function (main) {
        main.init();
    });
