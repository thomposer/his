        ...
        GridView::widget([
            ...
            'columns' => [
                ...
                [
                    ...
                    'template' => '{child-update}{child-delete}',
                    'buttons' => [
                        'child-update' => function ($url, $model, $key) {
                            if (!isset($this->params['permList']['role']) && !in_array($this->params['requestModuleController'] . '/child-update', $this->params['permList'])) {
                                return false;
                            }
                            return Html::a('修改', Url::to(['child-update', 'id' => $model->id]), ['title' => '修改','data-pjax' => 0]);
                        },
                        'child-delete' => function ($url, $model, $key) {
                            if (!isset($this->params['permList']['role']) && !in_array($this->params['requestModuleController'] . '/child-delete', $this->params['permList'])) {
                                return false;
                            }
                            $options = array_merge([
                                'data-confirm' => false,
                            ...
                            return Html::a('删除', Url::to(['child-delete', 'id' => $model->id]), $options);
                        },
                            ]
                        ],
                    ],
