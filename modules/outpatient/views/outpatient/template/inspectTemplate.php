        ...
        <div class = 'row search-margin'>
            <div class = 'col-sm-2 col-md-2  case-top '>
                <?php if (isset($this->params['permList']['role']) || in_array($this->params['requestModuleController'] . '/case-create-template', $this->params['permList'])): ?>
                    <?= Html::a("<i class='fa fa-plus'></i>新增", ['@outpatientOutpatientInspectTemplateCreate'], ['class' => 'btn btn-default font-body2', 'data-pjax' => 0]) ?>
                <?php endif ?>
            </div>
        </div>
            ...
            'columns' => [
                ...
                [
                    ...
                    'value' => function($searchModel) {
                        return InspectTemplate::$getType[$searchModel->type];
                    }
                ],
                ...
                [
                    ...
                    'template' => '{inspect-template-update}{inspect-template-delete}',
                    'headerOptions' => ['class' => 'col-xs-2 col-sm-2 col-md-2'],
                    'buttons' => [
                        'inspect-template-update' => function($url, $model, $key) {
                            if ((!isset($this->params['permList']['role']) && !in_array($this->params['requestModuleController'] . '/inspect-template-update', $this->params['permList'])) || $model->type == 1) {
                                return false;
                            }
                            $options = array_merge([
                            ...
                            return Html::a('<span class="icon_button_view fa fa-pencil-square-o" title="修改", data-toggle="tooltip"></span>', $url, $options);
                                ...
                                'inspect-template-delete' => function($url, $model, $key) {
                            if ((!isset($this->params['permList']['role']) && !in_array($this->params['requestModuleController'] . '/inspect-template-delete', $this->params['permList'])) || $model->type == 1) {
                                return false;
                            }
                            $options = [
                                'data-confirm' => false,
                            ...
                            return Html::a('<span class="icon_button_view fa fa-trash-o" title="删除", data-toggle="tooltip"></span>', ['@outpatientOutpatientInspectTemplateDelete', 'id' => $model->id, 'type' => $model->type], $options);
                        },
                            ]
                        ],
                    ],
