...
if ($data['type'] == 1) {//非弹窗:上传,预览或者弹窗：上传
    ...
    $pluginEvents = [
        ...
        'filepreupload'=>"function(event, data, id, index){
            var hiddenMessage=$('.message-attachment').val();
            if(hiddenMessage){
                 return {
                    message: '选择的上传文件个数 (2) 超出最大文件的限制个数 1.',
        ...
        "fileuploaded" => "function (event, data, id, index) {
                            ...
                            $('.message-attachment').val(json.path);
        }",
        //选择文件后的回调方法
        'filebatchselected' => "function(event,data){
         ...
         //删除文件后的回调方法
        'filesuccessremove'=>  "function(event, id, index){
