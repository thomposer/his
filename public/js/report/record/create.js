    ...
    var main = {
        init: function () {
            ...
            $('body').on('click', '.J-name-search-submit', function () {
                ...
                window.location.href = createUrl + '?id=' + id + '&url=' + url;
            });
                ...
                if (val) {
                    $.ajax({
                        ...
                        success: function (json) {
                            ...
                            if (appointmentType == 0 && action == 'update' && error == 0) {
                            ...
                            if (deleteStatus != 1) {
                                if (result.length >= 1) {
                                    for (key in result) {
                                        ...
                                        if (appointmentType == 0 && result[key]['id'] == spotTypeId && error == 1) {
                                            a += '<option value="' + result[key]['id'] + '" selected>' + htmlEncodeByRegExp(result[key]['type']) + '</option>';
                                            continue;
                                        } else if (appointmentType == 0 && result[key]['id'] == spotTypeId && action == 'update' && error == 0) {
                                            hasType = 1;
                                            console.log(result[key]['id']);
                                            continue;
                                        }else if(result.length == 1){
                                        	$('.field-report-type').removeClass('has-error').addClass('has-success');
                                        	$('.field-report-type .help-block').html('');
                                            a += '<option value="' + result[key]['id'] + '" selected>' + htmlEncodeByRegExp(result[key]['type']) + '</option>';
                                        	continue;
                                        }
                                    ...
                                    if (action == 'update' && appointmentType == 0 && error == 0) {
                        ...
                        if (count >= 1) {
                            $.each(result, function (key, val) {
                                ...
                                $.each(val.children, function (key, value) {
                                	if (firstDepartment == 0 && value['id'] == secondDepartmentId && error == 1) {
                                        a += '<option value="' + value['id'] + '" selected>' + htmlEncodeByRegExp(value['name']) + '</option>';
                                    } else if (firstDepartment == 0 && value['id'] == secondDepartmentId && action == 'update' && error == 0) {
                                    	hasType = 1;
                                        a += '<option value="' + value['id'] + '" selected>' + htmlEncodeByRegExp(value['name']) + '</option>';
                                    }else if(count == 1){
                            ...
                            if (action == 'update' && firstDepartment == 0 && error == 0) {
                                a = '<option value="' + secondDepartmentId + '" selected>' + secondDepartmentName + '</option>';
                                main.setAppointmentDisabled();
                            }
                        }
                    }
                    if (hasType == 0 && firstDepartment == 0) {
                        a = '<option value="' + secondDepartmentId + '" selected>' + secondDepartmentName + '</option>';
                        main.setAppointmentDisabled();
                    }
...
//                var errorData = $('#patient-username').siblings('.help-block').html();
//                $('#patient-username').parent().removeClass('has-success');
//                $('#patient-username').siblings('.help-block').html('')
//                $('#patient-hourmin').parent().removeClass('has-success');
//                $('#patient-hourmin').siblings('.help-block').html('')
                $('#report-patient').yiiAjaxForm({
                    ...
                    success: function (data) {
                        ...
                        var actionUrl = $('#reportForm').attr('actionurl');
                        if (data.similarUser) {
                            var data = {postParam: data.postParam, similarUser: data.similarUser, actionUrl: actionUrl};
                            modal.open('#reportForm', data);
                            isCommitted = false;
                            return false;
                        } else if (data.errCode == 1001) {
                            if (data.patientError) {
                                for (var x in data.patientError) {
                                    var obj = $('.field-patient-' + x.toLowerCase());
                                    console.log(data.patientError[x],'xxx');
//                                    parentObj.hasClass('has-success') ? parentObj.removeClass('has-success').addClass('has-error') : parentObj.addClass('has-error');
                                    obj.removeClass('has-success').addClass('has-error');
                                    obj.find('.help-block').html(data.patientError[x]);
                                }
                            }
                            if(data.reportErrors){
                                for (var y in data.reportErrors) {
                                    var target = $('.field-report-' + y.toLowerCase());
                                    target.removeClass('has-success').addClass('has-error');
                                    target.find('.help-block').html(data.reportErrors[y]);
                                }
                            }
                            isCommitted = false;
                        } else {
