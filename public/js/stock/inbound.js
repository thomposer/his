...
define(function (require) {
    ...
    var inboundTpl = require('tpl/stock/inbound.tpl');
    ...
    var main = {
        ...
        bindEvent: function () {
            $('.select2').select2();
            totalPriceMain.init();
            $('body').on('change', '#consumablesstockinfo-consumablesname', function (e) {
                var val = $(this).val();
                _self.addRecipe(val);
                $(this).val('');
            });
            $('body').on('click', '.consumables-stock-info .op-group>img', function () {
                $(this).parents('tr').hide();
                $(this).siblings('input[name="ConsumablesStockInfo[deleted][]"]').val(1);
        ...
        addRecipe: function (id) {
            var list = consumablesList[id];
            var inboundModel = template.compile(inboundTpl)({
                list: list,
                baseUrl: baseUrl,
            });
            $('.consumables-stock-info tbody').append(inboundModel);
