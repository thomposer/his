...
define(function (require) {
    ...
    var inboundTpl = require('tpl/stock/outbound.tpl');
    ...
    var main = {
        ...
        bindEvent: function () {
            $('.select2').select2();
            $('body').on('change','#consumablesoutboundinfo-consumablesname',function(e){
                var val = $(this).val();
                _self.addConsumables(val);
                $(this).val('');
            });
            $('body').on('click','.outbound-info .op-group>img',function(){
                $(this).parents('tr').hide();
                $(this).siblings('input[name="ConsumablesOutboundInfo[deleted][]"]').val(1);
            });
